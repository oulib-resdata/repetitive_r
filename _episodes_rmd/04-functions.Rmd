...
Let's define a function `conversion()` that converts values into a new value 2.14 times the original value.

```{r}
conversion <- function(value) {
  converted <- value*2.14
  return(converted)
}
```
...
You don't have to, but we're going to write our functions into a separate R script. You can load in the functions into our R session by using the
`source()` function:
