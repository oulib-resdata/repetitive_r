...
objectives:
- understand vectorized functions as applied to sample data
- use loops to repeat tasks based on row/column/element indexing
...
Vectorization (lapply and friends), loops, and custom functions.

## When should you use each case?
R is vectorized which means most actions can be performed by applying a function to a column by itself with no further work.  lapply() and its related functions (tapply(), sapply()) let you apply functions in groups.

Loops are useful for modifying an object repeatedly in place and recursive functions where order matters.
