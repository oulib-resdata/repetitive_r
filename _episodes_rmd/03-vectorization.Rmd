...
```{r}
unlist(lapply(X = mtcars, FUN = class))
```

** Divide each column by the mean **
```{r}
means_mtcars <- mtcars
means_mtcars[] <- lapply(X = mtcars[2:3], 
                   FUN = function(y) y / mean(y)) #y is an arbitrary variable used only, here, change to demo with x, z, etc.
mtcars_lapply <- lapply(X = mtcars[,2:3],
                   function(y) y / mean(y))  #you can also assign to a new object, by default is a list.
```
...
Remember a dataframe is a specialized list; other useful functions are mean(), unlist(),  and length().

## Solution to challenge 2
``` {r}
#how to pull a column?
...
#The null in [[1]] can be useful in some contexts if you have missing data, not in others.
```

**`tidyverse` solutions to lapply exist but we won't cover them here.** 
